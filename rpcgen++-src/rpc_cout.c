...
extern int cpp_mode;
extern int csharp_mode;

void emit_namespace(int After);

static void print_header(definition *);
static void print_trailer(void);
static void EmitConstructor(definition *);
static void EmitDestructor(definition *);
static void EmitXdr(definition *);
...
/**
 * Emit the Constructor for the object.
 */
void EmitConstructor(definition * def)
{
  decl_list *dl;
  ...
  for (dl = def->def.st.decls; dl != NULL; dl = dl->next) {
    ...
    if (streq(dl->decl.type, "string")) {
      f_print(fout, "\t%s = nullptr;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "double")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0.0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "float")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0.0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "int64_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "int32_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "int16_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "int8_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "uint64_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "uint32_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "uint16_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "uint8_t")
	       && dl->decl.rel != REL_ARRAY) {
      f_print(fout, "\t%s = 0;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "opaque")) {
      f_print(fout, "\t%s.Len = 0;\n", dl->decl.name);
      f_print(fout, "\t\t%s.Data = nullptr;\n", dl->decl.name);

    } else if (streq(dl->decl.type, "array")) {
      f_print(fout, "\t%s.Len = 0;\n", dl->decl.name);
      f_print(fout, "\t%s.Data = nullptr;\n", dl->decl.name);

    } else if (dl->decl.rel == REL_ARRAY) {
      f_print(fout, "\t%s.Len = 0;\n", dl->decl.name);
      f_print(fout, "\t%s.Data = nullptr;\n", dl->decl.name);

    } else {
      if (dl->decl.rel == REL_POINTER) {
	f_print(fout, "\t%s = nullptr;\n", dl->decl.name);

      } else if (dl->decl.rel == REL_ALIAS) {
				//f_print(fout, "\tmemset(&%s,0, sizeof(%s));/**DD*/\n",
				////"\t%s = (%s)0;\n",
				//dl->decl.name, dl->decl.type, dl->decl.name);

      } else {
	//printf("Constructor did not process: %s\n",
	//dl->decl.type);
	f_print(fout,
					"\tmemset(&%s,0, sizeof(%s));/**EE*/\n",
		dl->decl.name, dl->decl.name);
...
/**
 * Emit the Destructor for the object.
 */
void EmitDestructor(definition * def)
{
  decl_list *dl;
  ...
  for (dl = def->def.st.decls; dl != NULL; dl = dl->next) {
    ...
    if (streq(dl->decl.type, "string")) {
			if (cpp_mode) {
				f_print(fout, "\tif (%s != nullptr) {\n", dl->decl.name);
				f_print(fout, "\t\tdelete[] %s;\n", dl->decl.name);
				f_print(fout, "\t\t%s = nullptr;\n", dl->decl.name);
				f_print(fout, "\t}\n", dl->decl.name);
			} else if (csharp_mode) {
				f_print(fout, "\t\t%s = \"\";\n", dl->decl.name);
      ...
      f_print(fout, "\t%s = 0;\n", dl->decl.name);
			...
			if (cpp_mode) {
				f_print(fout, "\n\t%s.Len = 0;\n", dl->decl.name);
				f_print(fout, "\tif (%s.Data != nullptr) {\n", dl->decl.name);
				f_print(fout, "\t\tdelete[] %s.Data;\n", dl->decl.name);
				f_print(fout, "\t\t%s.Data = nullptr;\n", dl->decl.name);
				f_print(fout, "\t}\n", dl->decl.name);
			} else if (csharp_mode) {
				f_print(fout, "\n\t\tArray.Clear(%s.Data, 0, %s.Len);\n", dl->decl.name);
				f_print(fout, "\t\t%s.Len = 0;\n", dl->decl.name);
			}
      ...
      if (dl->decl.rel == REL_POINTER) {
				...
				if (cpp_mode) {
					//f_print(fout,
					//"\tmemset(&%s,0, sizeof(%s));/**FF*/\n",
					//dl->decl.name, dl->decl.name);
				}

      } else {
				//printf("Destructor did not process: %s\n", dl->decl.type);
				if (cpp_mode) {
					f_print(fout,
									"\tmemset(&%s,0, sizeof(%s));/**GG*/\n",
									dl->decl.name, dl->decl.name);
				}
      }
    }
...
/**
 * Emit the Xdr method for the object.
 */
void EmitXdr(definition * def)
{
  emit_namespace(0);
  f_print(fout, "\nint\n");
	if (cpp_mode) {
		f_print(fout, "%s::Xdr(XDR & xdrs)\n", def->def_name);
	} else if (csharp_mode) {
		f_print(fout, "Xdr(XDR & xdrs)\n", def->def_name);
	}
  f_print(fout, "{\n");
	if (cpp_mode) {
		f_print(fout, "\treturn(xdr_%s(&xdrs, this));\n", def->def_name);
	} else if (csharp_mode) {
		f_print(fout, "\treturn(xdr_%s(xdrs, this));\n", def->def_name);
	}
  f_print(fout, "}\n");
  ...
  //  f_print(fout, "\nint\n");
  //  f_print(fout, "xdr_bool(XDR * xdrs, bool * Obj)\n");
  //  f_print(fout, "{\n");
  //  f_print(fout, "\treturn(xdr_bool(xdrs, (bool_t*)Obj));\n", def->def_name);
  //  f_print(fout, "}\n");
...
static void print_ifopen(int indent, char *name)
{
  ...
  if (streq(name, "rpcprog_t") ||
      ...
      streq(name, "rpcprot_t") || streq(name, "rpcport_t")) {
    (void) strtok(name, "_");
  }
...
static void print_ifsizeof(int indent, char *prefix, char *type)
{
  ...
  if (streq(type, "bool")) {
    f_print(fout, "sizeof (bool), (xdrproc_t)xdr_bool");
			 ...
			 char *amax, char *objname, char *name)
{
  ...
  case REL_ARRAY:
	...
	print_ifopen(indent, "array");
      ...
      print_ifarg("(char **)");
      if (*objname == '&') {
	f_print(fout, "%s.Data, (uint32_t *) %s.Len", objname,
		objname);

      } else {
	f_print(fout,
		"&%s->Data, (uint32_t *) &%s->Len", objname, objname);
      }
    }
...
static void emit_union(definition * def)
{
  ...
  for (cl = def->def.un.cases; cl != NULL; cl = cl->next) {
	...
	+ strlen("&objp->%s_u.%s") + strlen(cs->name) + 1;

      object = malloc(len);

      if (isvectordef(cs->type, cs->rel)) {
	(void) snprintf(object,
			len, "objp->%s_u.%s", def->def_name,
			cs->name);

      } else {
	(void) snprintf(object,
			len,
			"&objp->%s_u.%s", def->def_name, cs->name);
      }

      print_ifstat(2,
		   cs->prefix,
		   cs->type, cs->rel, cs->array_max, object,
		   cs->name);
      free(object);
    }
    f_print(fout, "\t\tbreak;\n");
  ...
  if (dflt != NULL) {
    if (!streq(dflt->type, "void")) {
      size_t len = strlen(def->def_name)
	+ strlen("&objp->%s_u.%s") + strlen(dflt->name) + 1;
	...
	(void) snprintf(object,
      ...
      print_ifstat(2,
		   dflt->prefix,
		   dflt->type,
		   dflt->rel, dflt->array_max, object, dflt->name);
...
static void expand_inline(int indent,
			  const char *sizestr,
			  int size,
			  int flag, decl_list * dl, decl_list * cur)
{
  decl_list *psav;
 ...
 */
static int inline_type(declaration * dc, int *size)
{
  bas_type *ptr;
...
static char *arraysize(char *sz, declaration * dc, int elsize)
{
  int len;
  int elsz = elsize;
  int digits;
  ...
  if (elsize == 1) {
    (void) snprintf(tmp + slen, tlen - slen, "%s%s", plus,
		    dc->array_max);
  } else {
...
static void inline_struct(decl_list * dl,
			  decl_list * last, int flag, int indent)
{
  int size, tsize;
  decl_list *cur;
  char *sizestr;
 ...
 */
static int check_inline(decl_list * dl, int inlinelen, int *have_vector)
{
  int tsize = 0;
  int size;
  int doinline = 0;
...
static void emit_struct_tail_recursion(definition * defp, int can_inline)
{
  int indent = 3;
  ...
  f_print(fout, "\n\t\t\tif (!more_data) {\n");
  f_print(fout, "\t\t\t\tobjp->%s = nullptr;\n", sp->tail->decl.name);
  f_print(fout, "\t\t\t\tbreak;\n");
  f_print(fout, "\t\t\t}\n\n");
  f_print(fout, "\t\t\tif (objp->%s == nullptr) {\n",
	  sp->tail->decl.name);
  f_print(fout,
	  "\t\t\t\tobjp->%s = "
	  "(%s *)\n\t\t\t\t\tmem_alloc(sizeof (%s));\n",
...
static void emit_inline64(int indent, declaration * decl, int flag)
{
  switch (decl->rel) {
...
static void emit_single_in_line(int indent,
				declaration * decl, int flag, relation rel)
{
  char *upp_case;
  int freed = 0;
...
static void emit_single_in_line64(int indent,
				  declaration * decl,
				  int flag, relation rel)
{
  char *upp_case;
  int freed = 0;
